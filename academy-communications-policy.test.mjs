...
const moduleUrl=new URL('./supabase/functions/academy-communications/communication-policy.mjs',import.meta.url)
const modulePath=fileURLToPath(moduleUrl)

test('communication policy module exists',()=>{
  assert.equal(existsSync(modulePath),true,'communication-policy.mjs must exist')
})

if(existsSync(modulePath)){
  const {SENDERS,eventKey,indiaDateKey,maskRecipient,normalisePhone}=await import(moduleUrl)
