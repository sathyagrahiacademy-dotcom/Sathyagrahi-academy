...
let studentId=null,subjectCache={},syllabusCache={};
...
function renderTopic(topic,examRows){
  ...
  return `<div class="topic-exam-row" data-topic="${esc(topic.id)}">
    <div class="topic-exam-name">
      ...
      <small>${exams.length?`${exams.length} exam${exams.length===1?'':'s'} · ${attempted} attempted`:'No exam created for this chapter yet'}</small>
    </div>
...
function renderUnit(unit,examRows){
  const allTopicExams=unit.topics.flatMap(t=>topicExams(examRows,t.id));
  const testedTopics=unit.topics.filter(t=>topicExams(examRows,t.id).length>0).length;
  const remaining=Math.max(0,unit.topics.length-testedTopics);
  const totalExams=uniqExams(allTopicExams).length;
  ...
  return `<section class="unit-exam-card">
    <div class="unit-exam-head" role="button" tabindex="0">
      <div class="unit-exam-title">
        ...
        <div class="unit-compact-note">Click to view chapters and E1/E2/E3 exams</div>
      </div>
    ...
    <div class="topic-exam-list">
      ${unit.topics.length?unit.topics.map(t=>renderTopic(t,examRows)).join(''):'<div class="exam-empty">No chapters found.</div>'}
    </div>
  </section>`;
}
...
async function loadSubject(subject){
  ...
  try{
    ...
    const testedTopics=allTopics.filter(t=>topicExams(examRows,t.id).length>0);
    const totalExams=uniqExams(examRows.filter(e=>e.topic_id)).length;
    ...
    <div class="chapter-exam-list">
      ${units.map(u=>renderUnit(u,examRows)).join('')}
    </div>`;
  }catch(err){
