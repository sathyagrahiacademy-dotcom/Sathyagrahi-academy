(function(root,factory){
  ...
  function formatEHistoryRow(row={}){const pct=Number(row.percentage||0);const score=`${num(row.earned_marks)}/${num(row.max_marks)}`;return `E${num(row.exam_sequence)||1} • ${String(row.coverage||'partial').toUpperCase()} • ${num(row.question_count)}Q • ${score} • ${pct.toFixed(1)}%`;}
  ...
  return{scopeKey,groupPerformanceByScope,formatEHistoryRow,filterScopeRows,buildStudentHierarchy,subjectCardModel,sortAttemptHistory,attemptLabel,subjectScopeHierarchy};
});
