...
function renderHierarchy(){
  ...
  $('performanceHierarchy').innerHTML=root.units.map((unit,index)=>`<section class="unit-card"><div class="unit-head"><div><span class="unit-kicker">UNIT ${unit.unitNo||index+1}</span><div class="unit-title">${esc(unit.title)}</div></div><div class="scope-history">${registerChips(unit.history)}</div></div>${unit.chapters.map(ch=>`<div class="chapter-row"><div class="chapter-head"><div class="chapter-title">${esc(ch.title)}</div><div class="scope-history">${registerChips(ch.history)}</div></div>${ch.subtopics.map(topic=>`<div class="topic-row"><div class="topic-title">${esc(topic.title)}</div><div class="scope-history">${registerChips(topic.history)}</div></div>`).join('')}</div>`).join('')}</section>`).join('');
}
